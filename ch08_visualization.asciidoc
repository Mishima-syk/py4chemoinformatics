...
今回取り上げた事例では、化合物のFingerprintをPCAやtSNEを用いて次元圧縮した結果に企業名を載せると綺麗にマッピングができました。ですが、毎回情報があるとは限りません。このような場合、クラスタリングを行い化合物を適当なグループに分割します。
...
さて、ここまでで化合物の特徴を元にそれを低次元のケミカルスペースに投影する手法を学びました。
...
一般的にはPCAやtSNEでは既存のケミカルスペースに新しい化合物を投影できません。なぜならこれらの手法は全化合物で計算する必要があるからです。しかし、自己組織化マップ(SOM)を使うことで可能となります。ここでは発想を転換し、トレーニングデータを使い特徴量＝＞低次元空間への次元圧縮の予測モデルを作ります。こうすることで、既存のケミカルスペースに新しい化合物を投影できるようになります。これを実装してるパッケージがlink:http://www.xavierdupre.fr/app/mlinsights/helpsphinx/index.html[mlinsights]です。このパッケージはpipコマンドでインストールできます。
...
ここで一点注意としてfitを呼び出すときに本来不要なyに対応する値を渡す必要があり、下のコードではダミーの値としてただのIndexを与えています。
...
GPモデルはあまり性能が出ませんでした。PredictableTSNEは新しい化合物の既存スペースへの投影の一つのツールとなり得ますが、この例のようにモデルにより結果が大きく左右されることがあるようです。
