...
構造を見てみましょう。二つの構造を見てみると中心の環構造部分の窒素原子の並びが異なるものの、極めて似ている構造のように見えませんか？両分子は同じ標的タンパク質をそがしますが、その活性や薬物動態は異なります。ちょっとの違いで選択性や薬物動態が異なるのはとても興味深いですね。

image::ch05/ch05_08.png[check structures]

上のイメージを作るサンプルコードは以下の通りです。
以下の例ではCompute2DCoordsでSmilsから取得したmol objectに二次元の配座情報を付与し、別途用意したCore構造をベースにアライメントしています。
Draw.MolToGridImageのオプションにlegendsを与え、分子名を描画しています。
