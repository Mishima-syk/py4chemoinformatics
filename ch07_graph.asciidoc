...
読み込んだ分子を使ってMCSを取得してみましょう。RDKitにはMCSの取得方法に複数のオプションが指定できます。以下の例では、1デフォルト、2原子がなんであっても良い（構造とボンドの次数があっていれば良い）、3結合次数がなんでも良い（例えば、ベンゼンとシクロヘキサンは同じMCSとなる）という例を示します。
