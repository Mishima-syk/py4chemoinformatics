...
別のファイルで試したい場合はファイルパスの部分を適宜変更してください。
...
MCSは分子同士の最大公約数と言えます。つまり、MCSが大きいほどその分子同士は似ている言えます。しかしながら谷本係数のような数値として評価する指標はあまりありません。MCSに基づく類似性がスコア化できたらクラスタリングや、それ以外の解析においても有用では無いでしょうか。
幸運なことに、RDKitにはMCSのコンセプトに近い、細かな変化には影響を受けにくい類似性評価のアルゴリズムFraggle Similarityというものも実装されています。以下に簡単ではありますがその利用例を示します。
